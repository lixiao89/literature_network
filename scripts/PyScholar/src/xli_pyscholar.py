...
scholar.py -c 5 -a "albert einstein" -t --none "quantum theory" --after 1970"""
    ...
    group.add_option('-l', '--layers', type='int', default=3,
                     help='Get three layers of backward citing literature')
    ...
    # get three layers of backward citing results
    if options.layers is not None:
        try:
            loop(options, query, querier, file_name = '../results/layer' + str(1) + '/layer1.json')
            reset_res()
            for i in range(2, options.layers + 1):
                if i == 2:
                    with open('../results/layer' + str(i-1) + '/layer' + str(i-1) + '.json') as data_file:
                        urls = json.load(data_file)
                    if isinstance(urls, list) and len(urls) > 0 and isinstance(urls[0], dict):
                        urls = [x['url_citations'] for x in urls]
                    for url in urls:
                        query.set_url(url)
                        reset_res()
                        loop(options, query, querier, file_name='../results/layer' + str(i) + '/' + 
                             re.match('.*?([0-9]+)', url).group(1) + '.json')
                else:
                    for file_name in os.listdir('../results/layer' + str(i-1) + '/'):
                        with open('../results/layer' + str(i-1) + '/' + file_name) as data_file:
                             urls = json.load(data_file)
                        if isinstance(urls, list) and len(urls) > 0 and isinstance(urls[0], dict):
                            urls = [x['url_citations'] for x in urls]
                        for url in urls:
